...
- (BOOL)isEqual:(id _Nullable)object SWIFT_WARN_UNUSED_RESULT;
@property (nonatomic, readonly) NSUInteger hash;
/// Method to convert a dictionary to the our object, when we do not have a decodable
+ (NamiCampaign * _Nonnull)fromDictionary:(NSDictionary<NSString *, id> * _Nonnull)dict SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// Sent to server and available for all devices the user owns for this application - requires the user to be logged in.
  NamiTagPolicyApp = 2,
/// Sent to server and available for all devices the user owns across all of the applications from developer of the application setting this tag.
  NamiTagPolicyGlobal = 3,
};
