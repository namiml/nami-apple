...
/// Internal Nami identifier for this user, in case you wish to do some cross checking on your server with data being sent by Nami.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull namiIdentifier;)
+ (NSString * _Nonnull)namiIdentifier SWIFT_WARN_UNUSED_RESULT;
...
+ (void)registerHandlerWithHandler:(void (^ _Nullable)(enum NamiAnalyticsActionType, NSDictionary<NSString *, id> * _Nonnull))handler;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end


SWIFT_CLASS("_TtC4Nami19NamiCampaignContext")
...
SWIFT_CLASS("_TtC4Nami19NamiCampaignManager")
...
@interface NamiCampaignManager (SWIFT_EXTENSION(Nami))
...
///
+ (void)launchWithLabel:(NSString * _Nullable)label waitOnData:(double)waitOnData launchHandler:(void (^ _Nullable)(BOOL, NSError * _Nullable))launchHandler;
@end
...
/// \param changeHandler A callback called when customer journey changes in the system are detected.
///
+ (void)registerJourneyChangeHandler:(void (^ _Nullable)(CustomerJourneyState * _Nonnull))changeHandler;
...
///
+ (void)loginWithId:(NSString * _Nullable)id type:(enum NamiExternalIdentifierType)type;
...
enum NamiEntitlementType : NSInteger;
...
/// Provides the Nami SDK with an entitlement your application grants from sources outside the users current device.  Please ensure the entitlement names match the ones you define in the Nami Control Center for your application.  Any changes with entitlement reference IDs that do not match will be discarded.
/// Any entitlement granted will stay active until you pass in a new entitlement, or it is revoked by the server or a call to <code>revokeExternal</code>
/// \param referenceID Name of entitlment defined in Nami Control Center.
///
/// \param type Type of entitlement (default is binaryAuth).
///
/// \param expiresAt Timestamp that entitlement shoudl expire (if any).
///
+ (void)activateExternalWithReferenceId:(NSString * _Nonnull)referenceId type:(enum NamiEntitlementType)type expiresAt:(NSDate * _Nullable)expiresAt;
/// Provides the Nami SDK with an entitlement your application wishes to revoke.  Please ensure the entitlement names match the ones you define in the Nami Control Center for your application.  Any changes with entitlement reference IDs that do not match will be discarded.
/// Any entitlement granted will stay active until you pass in a new entitlement, or it is revoked by the server or a call to <code>revokeExternal</code>
/// \param referenceId Name of entitlment defined in Nami Control Center.
///
+ (void)revokeExternalWithReferenceId:(NSString * _Nonnull)referenceId type:(enum NamiEntitlementType)type expiresAt:(NSDate * _Nullable)expiresAt;
/// Unregisters any active entilement change handlers..
+ (void)unregisterChangeHandler;
...
///
+ (void)registerChangeHandler:(void (^ _Nonnull)(NSArray<NamiEntitlement *> * _Nonnull))changeHandler;
@end
...
SWIFT_CLASS("_TtC4Nami17NamiLanguageCodes")
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, copy) NSSet<NSString *> * _Nonnull allAvailiableNamiLanguageCodes;)
+ (NSSet<NSString *> * _Nonnull)allAvailiableNamiLanguageCodes SWIFT_WARN_UNUSED_RESULT;
+ (void)setAllAvailiableNamiLanguageCodes:(NSSet<NSString *> * _Nonnull)value;
/// Language Afrikaans
...
/// \param paywallClosedHandler A callback that wil be called if a Nami paywall is being closed.
///
+ (void)registerClosedHandler:(void (^ _Nullable)(BOOL, NamiCampaignContext * _Nullable, NamiPaywall * _Nullable))paywallClosedHandler;
...
SWIFT_CLASS("_TtC4Nami31NamiPaywallRoundableSpacingCell")
...
@interface NamiPurchaseManager (SWIFT_EXTENSION(Nami))
...
/// Finds the app receipt daat held in your local app bundle, returns any data found.
...
@interface SKProductSubscriptionPeriod (SWIFT_EXTENSION(Nami))
